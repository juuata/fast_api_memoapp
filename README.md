...
| Amazon ECS Fargate | - | バックエンドコンテナのサーバーレス実行 |
| Amazon ECR | - | Dockerイメージのプライベートレジストリ |
| Amazon RDS (PostgreSQL) | 16 | マネージドPostgreSQLデータベース |
...
[AWS Amplify] ← Reactフロントエンド（静的ホスティング）
    ↓ HTTP（※HTTPS化は今後の課題）
[Amazon ECS Fargate] ← FastAPIバックエンド（コンテナ実行）
    ↓
...
| バックエンドAPI | http://13.231.214.36:8000 |
| APIドキュメント | http://13.231.214.36:8000/docs |

### 今後の改善点

- HTTPS化：ALB（Application Load Balancer）+ ACM（SSL証明書）+ Route 53（独自ドメイン）により対応予定
